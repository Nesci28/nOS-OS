module.exports = function(json, step, tables = "") {
  ...
  function showCoin(json, brand) {
    ...
    if (json[brand]["Coin Info"].wallet) {
      value = [
        ["Wallet", json[brand]["Coin Info"].wallet],
        ["URI", json[brand]["Coin Info"].poolUri1],
        ["Command", json[brand]["Coin Info"].command],
      ];
    } else {
      value = [
        ["Account", json[brand]["Coin Info"].account],
        ["URI", json[brand]["Coin Info"].poolUri1],
        ["Command", json[brand]["Coin Info"].command],
      ];
    }
  ...
  function showGPU(json, brand, gpuNumber, step) {
      ...
      },
      {
        ...
        formatter: function(value) {
          ...
          if (value < ocSettings["Nvidia"]["Max Temperature"] - 3) {
            ...
            value >= ocSettings["Nvidia"]["Max Temperature"] - 3 &&
            value <= ocSettings["Nvidia"]["Max Temperature"] + 3
          ) {
    ...
    if (step !== "init") {
      for (let i = 0; i < gpuNumber; i++) {
        value.push([
          json[brand].GPU[i].Name,
          json[brand].GPU[i].Temperature,
          json[brand].GPU[i]["Fan Speed"].split(" ")[0],
          json[brand].GPU[i].Watt,
          json[brand].GPU[i]["Core Clock"].split(" ")[0],
          json[brand].GPU[i]["Mem Clock"].split(" ")[0],
          json[brand].GPU[i].Hashrate.split(" ")[0],
        ]);
      }
    } else {
      for (let i = 0; i < gpuNumber; i++) {
