module.exports = async function(json, step, watchdogStatus) {
  ...
  if (step !== "init") {
    if (watchdogStatus["Watchdog"]["Nvidia"]["Utilization"] == "Initializing") watchdogStatus["Watchdog"]["Nvidia"]["Utilization"] = 0;
    if (watchdogStatus["Watchdog"]["Amd"]["Utilization"] == "Initializing") watchdogStatus["Watchdog"]["Amd"]["Utilization"] = 0;
    if (watchdogStatus["Watchdog"]["Nvidia"]["Core"] == "Initializing") watchdogStatus["Watchdog"]["Nvidia"]["Core"] = 0;
    if (watchdogStatus["Watchdog"]["Amd"]["Core"] == "Initializing") watchdogStatus["Watchdog"]["Amd"]["Core"] = 0;
  ...
  async function getTemp(infos) {
    ...
    function getGPUTemp(infos, brand) {
      for (let i = 0; i < infos[brand]["GPU"].length; i++) {
        ...
        } else if (gpuTemperature > maxTemp[brand]["Max Temperature"] + 3 && gpuTemperature < maxTemp[brand]["Max Temperature"] + 10) {
          watchdogStatus["Watchdog"][brand]["Temperature"][i] = (watchdogStatus["Watchdog"][brand]["Temperature"][i]+1) || 1
        } else if (gpuTemperature >= maxTemp[brand]["Max Temperature"] + 10) {
          watchdogStatus["Watchdog"][brand]["Temperature"][i] = (watchdogStatus["Watchdog"][brand]["Temperature"][i]+2) || 2
        }
  ...
  async function getMemClock(infos) {
    ...
    function getGPUMemClock(infos, brand) {
      for (let i = 0; i < infos[brand]["GPU"].length; i++) {
        if (brand === "Nvidia") {
          var gpuMemClock = infos[brand]["GPU"][i.toString()]["Mem Clock"].split(' ')[0]
          var gpuMemClockMax = infos[brand]["GPU"][i.toString()]["Max Mem"].split(' ')[0] - 300
        }
        if (brand === "Amd") {
        ...
        if (gpuMemClock > gpuMemClockMax) {
          ...
          watchdogStatus["Watchdog"][brand]["Mem"][i] = (watchdogStatus["Watchdog"][brand]["Mem"][i]+1) || 1
        }
  ...
  async function getCoreClock(infos) {
    ...
    function getGPUCoreClock(infos, brand) {
      for (let i = 0; i < infos[brand]["GPU"].length; i++) {
        if (brand === "Nvidia") {
          var gpuCoreClock = infos[brand]["GPU"][i.toString()]["Core Clock"].split(' ')[0]
          var gpuCoreClockMax = infos[brand]["GPU"][i.toString()]["Max Core"].split(' ')[0] - 1000
        }
        if (brand === "Amd") {
        ...
        if (gpuCoreClock > gpuCoreClockMax) {
          ...
          watchdogStatus["Watchdog"][brand]["Core"][i] = (watchdogStatus["Watchdog"][brand]["Core"][i]+1) || 1
        }
  ...
  async function getUtilization(infos) {
    ...
    function getGPUUtilization(infos, brand) {
      for (let i = 0; i < infos[brand]["GPU"].length; i++) {
        if (brand == "Nvidia") var gpuUtils = infos[brand]["GPU"][i.toString()]["Utilization"].split(' ')[0]
        if (brand == "Amd") var gpuUtils = infos[brand]["GPU"][i.toString()]["Utilization"].toString()
        
        if (gpuUtils >= 90) {
          watchdogStatus["Watchdog"][brand]["Utilization"][i] = 0
        } else {
          watchdogStatus["Watchdog"][brand]["Utilization"][i] = (watchdogStatus["Watchdog"][brand]["Utilization"][i]+1) || 1
        }
