module.exports = async function(json, step) {
  ...
  if (nvidiaGpuNumber > 0 && step == "init")
    initialize(json, nvidiaGpuNumber, "Nvidia");
  if (amdGpuNumber > 0 && step == "init") initialize(json, amdGpuNumber, "Amd");

  // Stop the fans
  if (nvidiaGpuNumber > 0 && step == "stop")
    stop(json, nvidiaGpuNumber, "Nvidia");
  ...
  function initialize(infos, gpuNumber, brand) {
    ...
    for (let i = 0; i < gpuNumber; i++) {
      if (brand == "Nvidia") {
        initCommand +=
          "sudo nvidia-settings -a [gpu:" +
          i +
          "]/GPUFanControlState=1 -a [fan:" +
          i +
          "]/GPUTargetFanSpeed=" +
          fanSpeed +
          "; ";
      ...
      for (let i = 0; i < infos[brand]["GPU"].length; i++) {
        if (brand == "Nvidia")
          currentFanSpeed = Number(
            infos[brand]["GPU"][i]["Fan Speed"].split(" ")[0],
          );
        if (brand == "Amd") {
        ...
        if (infos[brand]["GPU"][i.toString()]["Temperature"] < minTemperature) {
          if (currentFanSpeed >= 21) {
            ...
            if (brand == "Amd") {
              fanCommand += `sudo ./helpers/ROC-smi/rocm-smi -d ${amdGpuID} --setfan ${currentFanSpeed -
                5}%; `;
              gpuTemperature[i] = currentFanSpeed - 5;
            }
          }
        } else if (
          infos[brand]["GPU"][i.toString()]["Temperature"] > maxTemperature
        ) {
          if (brand == "Nvidia") {
          ...
          if (brand == "Amd") {
            if (currentFanSpeed <= maxFanSpeed - 5) {
              ...
              fanCommand += `sudo ./helpers/ROC-smi/rocm-smi -d ${amdGpuID} --setfan ${maxFanSpeed +
                2}%; `;
              gpuTemperature[i] = maxFanSpeed + " Max";
            }
          }
          ...
          if (currentFanSpeed == maxFanSpeed)
            currentFanSpeed = currentFanSpeed + " Max";
          gpuTemperature[i] = currentFanSpeed;
        }
