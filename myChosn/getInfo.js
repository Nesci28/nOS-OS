module.exports = async function(step) {
    ...
    await getCoins('Nvidia')
	  if (step) await getGPU('Nvidia')
	  if (step) await getHashrate('Nvidia', json['Nvidia']['Coin Info']['miner'])
  }
	if (systemConfig["Amd Coin"] && setType[1]) {
    await getCoins('Amd')
	  if (step) await getGPU('Amd')
    if (step) await getHashrate('Amd', json['Amd']['Coin Info']['miner'])
  }
  if (systemConfig["Cpu Coin"] && setType[2]) {
    await getCoins('Cpu')
	  if (step) await getGPU('Cpu')
    if (step) await getHashrate('Cpu')
  }
  return json
